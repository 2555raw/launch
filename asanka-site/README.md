...
No build step, no dependencies. Plain HTML, CSS and vanilla JS. Everything is in English.

The folder holds two pages that share `art.js`: the store (`index.html`) and
**Asanka Pad** (`launchpad.html`), a demo launchpad for coins paired with
African dishes.
...
3. "Review and send order" checks the form and shows a WhatsApp button that opens
...
- **Privacy.** "Accept and continue" lets the page keep the cart, the allergies and the
  theme in `localStorage`. "Necessary only" keeps them in memory for this visit only;
...
The footer's "Privacy" and "Allergens" links, and "Change" in the bar, reopen it.

## Vegan

The "Vegan" section and menu tab list the dishes flagged `vegan: true`: garden jollof,
...
- **The contact details are placeholders**: the WhatsApp number `34600000000`, the phone,
  `orders@asanka.example` and the address in `CONFIG`. The order goes to that number,
  so change it first.
- Prices, hours, the "45 min" and "0 preservatives" figures in the hero and the delivery
...
- There is no online payment: the site sends the order through WhatsApp and payment
  happens on delivery (cash, card or bank transfer, as the FAQ says).
