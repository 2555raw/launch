...
A working copy of heyaskr.ai: the landing page, the `/ask` app and the API
...
crypto and card deposits, and the $ASKR holder allowance.
...
askr-site/
  ...
  lib/
    ...
    markets.js         crypto tickers (CoinGecko) and the $ASKR price (DexScreener)
    chain.js           $ASKR holdings, deposit verification (ETH/USDT, SOL, BTC), Stripe
...
**Landing** — the $ASKR pill and the crypto ticker refresh every minute;
...
**$ASKR** — link a wallet and the server reads the balance on Robinhood Chain
(`ROBINHOOD_RPC_URL` + `ASKR_TOKEN_ADDRESS`). Holders pay **5%** of list
