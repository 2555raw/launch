/* Live prices for the ticker and the $ASKR pill. Fetched from public APIs,
...
let askr = { at: 0, stale: true, price: 0.003828, change: 16.9, symbol: 'ASKR' };
...
async function refreshAskr() {
  if (Date.now() - askr.at < TTL) return askr;
  if (!config.chain.dexscreenerPair) { askr.at = Date.now(); return askr; }
  try {
    const j = await fetchJson(`https://api.dexscreener.com/latest/dex/pairs/${config.chain.dexscreenerPair}`);
    const p = j.pairs?.[0] || j.pair;
    if (p) askr = { at: Date.now(), stale: false, price: Number(p.priceUsd), change: Number(p.priceChange?.h24 || 0), symbol: 'ASKR', url: p.url };
  } catch {
    askr.at = Date.now() - TTL + 15000;
  }
  return askr;
}
...
async function snapshot() {
  const [t, a] = await Promise.all([refreshTickers(), refreshAskr()]);
  return { tickers: t, askr: a };
}
