...
(() => {
  const { api, getKey, setKey, usd, cr, pct, esc, mark, vendorMark, theme, toast } = askr;
  ...
  function renderSignbox() {
    ...
    const a = state.me;
    b.innerHTML = `<span class="mono">BALANCE</span><b>${cr(a.balance)} credits</b><small>${a.wallet ? a.wallet.slice(0, 6) + '…' + a.wallet.slice(-4) : 'access key'}${a.tier.holder ? ` · ${a.holdingsPct.toFixed(3)}% $ASKR` : ''}</small>`;
  ...
  function renderHome() {
    ...
    stage.innerHTML = `
      ${inChat ? `<div class="shared-h"><div><div class="stage-logo" style="font-size:22px;margin:0"><b style="font-size:26px">&gt;</b>askr</div><div class="note mono">${esc(state.chat.title)}</div></div><div class="cta-row"><button class="btn btn-ghost btn-sm" id="shareBtn">Share</button><button class="btn btn-ghost btn-sm" id="newBtn">New chat</button></div></div>` :
        `<div class="stage-logo"><b>&gt;</b>askr</div><h1>What are we <span class="accent" id="modeWord">experimenting?</span></h1>`}
  ...
  function renderThread() {
    ...
    t.innerHTML = msgs.map((m, i) => m.role === 'user'
      ...
      : `<div class="msg ai"><span class="who">${esc(m.model || 'askr')}</span><div class="body ${m.streaming ? 'cursor' : ''}">${m.streaming ? esc(m.content) : md(m.content)}</div>${m.streaming ? '' : `<div class="meta"><span>${m.credits !== undefined ? `−${cr(m.credits)} cr` : ''}</span>${m.usage ? `<span>${m.usage.inTokens}→${m.usage.outTokens} tok</span>` : ''}${m.saved ? `<span class="accent">saved ${cr(m.saved)} cr as a holder</span>` : ''}${m.live === false ? '<span class="demo">demo</span>' : ''}<button class="copy" data-say="${i}">${ICONS.speaker.replace('<svg', '<svg width="12" height="12" style="display:inline;vertical-align:-2px"')} listen</button><button class="copy" data-copytext="${i}">copy</button></div>`}</div>`).join('');
  ...
  function openSignIn() {
    ...
    $('#optKey').onclick = () => {
      $('#signExtra').innerHTML = `<div class="inline" style="display:flex;gap:8px;margin-top:12px"><input class="input" id="keyIn" placeholder="ask_…" autocomplete="off"><button class="btn btn-primary" id="keyGo">Sign in</button></div>`;
      $('#keyIn').focus();
  ...
  async function renderShared(token) {
    ...
    try {
      ...
      s.innerHTML = `<div class="view"><div class="shared-h"><div><h2>${esc(chat.title)}</h2><p class="sub" style="margin:0">Shared chat · ${chat.model} · ${new Date(chat.created).toLocaleDateString()}</p></div><a class="btn btn-primary btn-sm" href="#home">Start your own</a></div><div class="thread">${chat.messages.map((m) => m.role === 'user' ? `<div class="msg user"><span class="who">them</span><div class="body">${esc(m.content)}</div></div>` : `<div class="msg ai"><span class="who">${esc(m.model || 'askr')}</span><div class="body">${md(m.content)}</div></div>`).join('')}</div></div>`;
  ...
  async function renderAccount() {
    ...
    s.innerHTML = `<div class="view"><h2>Account</h2><p class="sub">${a.wallet ? a.wallet : 'Access-key account'} · since ${new Date(a.created).toLocaleDateString()}</p>
      ...
      <div class="panel"><h3>Top up</h3><p class="sub">$1 = ${cfg.creditsPerUsd.toLocaleString()} credits. No askr fee on the deposit: what you send is what you get.</p>
      ...
      <div class="panel"><h3>$ASKR holder tier</h3><p class="sub">Every model at 5% of its price on a daily allowance: 1,000 credits per 0.01% of supply held, up to 25,000. Resets 00:00 UTC.</p>
