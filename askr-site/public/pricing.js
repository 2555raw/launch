(() => {
  ...
  api('/api/models').then(({ categories, models }) => {
    ...
    const show = (cat) => {
      ...
      out.innerHTML = `<table class="table" id="${cat.id}"><thead><tr><th>Model</th><th>Vendor</th>${isChat ? '<th class="num">Input /1M</th><th class="num">Output /1M</th>' : '<th class="num">Price</th>'}<th class="num">$ASKR holders</th></tr></thead><tbody>${list.map((m) => `<tr><td><a href="/ask?model=${m.id}"><b>${esc(m.name)}</b></a>${m.badge ? `<span class="badge">${m.badge}</span>` : ''}${m.live ? '' : '<span class="badge demo">demo</span>'}<div class="id">${m.id}${m.context ? ` · ${(m.context / 1000).toLocaleString()}K ctx` : ''}</div></td><td>${esc(m.vendor)}</td>${isChat ? `<td class="num">${usd(m.prices.inUsd)}</td><td class="num">${usd(m.prices.usd)}</td>` : `<td class="num">${usd(m.prices.usd)} <span class="id">${m.prices.unit}</span></td>`}<td class="num accent">${usd(m.prices.holderUsd)}${isChat ? '' : ''}</td></tr>`).join('')}</tbody></table>`;
    };
