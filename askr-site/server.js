...
const SYSTEM = {
  ask: 'You are askr, a concise and capable assistant. Answer directly, use markdown when it helps, and keep code in fenced blocks.',
  code: 'You are askr in Code mode: a senior engineer. Give complete, runnable code in fenced blocks with the language tagged. When asked for a website or page, return a single self-contained HTML file. Explain only what is not obvious from the code.'
};
...
const api = {
  ...
  'GET /api/config': async () => ({
    ...
    chain: { id: config.chain.rhChainId, token: config.chain.askrToken, buyUrl: config.chain.buyUrl, chartUrl: config.chain.chartUrl },
    links: config.links
  }),
...
server.listen(config.port, () => {
  ...
  console.log(`askr on :${config.port} — ${live.length ? 'live: ' + live.join(', ') : 'no provider keys'}${config.demoAllowed() ? ' (demo mode for the rest)' : ''}`);
});
