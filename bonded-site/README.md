...
button, the floating navigation pill, a one-line bridge ("A coin can start with a joke…") into the pairs, and a scene behind the whole page that people can play with, the
...
- `config.js` holds `BONDED_OWNER` (the only wallet that can claim fees; live launches send the
  2% creator tax there) and `BONDED_STOCK_TOKENS`, a list of candidate
