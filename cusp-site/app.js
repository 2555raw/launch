...
/* ---------- the demo wallet ----------
   Play money, kept in this browser. Nothing is signed, no chain is touched and
   no address here belongs to anyone. It exists so the page can be used rather
   than looked at. */

const KEY = 'cusp-demo';
const START_USDG = 25000;
let wallet = null;   // { addr, usdg, pos: { NVDA: shares, … } }
...
function load() {
  try {
    ...
    if (!w || typeof w.addr !== 'string') return;
    wallet = { addr: w.addr, usdg: Number(w.usdg) || 0, pos: w.pos || {} };
...
function connect() {
  wallet = { addr: newAddress(), usdg: START_USDG, pos: {} };
  save();
  paint();
  toast('Demo wallet connected with ' + money(START_USDG) + ' USDG of play money.');
}
