...
A piece for Cyania: title cards cut against footage of the real page, over an
original soundtrack. 1280x720, 30fps, H.264 + AAC.

```bash
# 1. stage a servable copy with the typefaces beside it, and serve it
python3 video/stage.py /tmp/cyania-serve
cp video/card.html /tmp/cyania-serve/
cd /tmp/cyania-serve && python3 -m http.server 8899
...
# 3. cut it
python3 video/build.py video/raw video/cards video/cyania.mp4
```
