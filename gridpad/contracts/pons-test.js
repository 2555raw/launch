...
const params = {
  name: "Grand Coulee Coin",
  symbol: "GC",
  logo: "",
  description: PONS.describe("GC", "Grand Coulee, United States of America", "Hydro, 6,809 MW, at 47.9575° N, 118.9773° W."),
...
eq(back[0].name, "Grand Coulee Coin", "the launch encodes and decodes its name");
eq(back[0].symbol, "GC", "and its symbol");
...
store.set("gridpad.pons.factory.46630", MOCK);
store.set("gridpad.pons.factory.4663", MOCK);
store.set("gridpad.pons.factory.1", MOCK);
...
store.set("gridpad.pons.factory.46630", "not an address");
eq(PONS.address(46630), null, "a stored value that is not an address is ignored rather than trusted");
store.delete("gridpad.pons.factory.46630");
store.delete("gridpad.pons.factory.4663");
store.delete("gridpad.pons.factory.1");
ok(!PONS.has(46630), "and with nothing stored the testnet has no Pons again");

console.log("\nthe pairing, through the only field Pons gives us\n");

const described = PONS.describe("GC", "Grand Coulee", null);
eq(PONS.sourceOf(described), "GC", "the ticker survives the round trip");
ok(described.includes("Grand Coulee"), "a person reads the station, not a tag");
ok(/no ownership of the plant/.test(described), "and the disclaimer travels with it on chain");
...
eq(PONS.sourceOf(PONS.describe("HELL", "Hellisheidi")), "HELL", "a second source, with no note");

console.log("\nthe curve the pages quote with\n");

/* Pons prices against a reserve that includes a phantom amount nobody
 * deposited, exactly the shape of Gridpad's own virtual reserve. */
...
ok(/viaPons\(\)/.test(chainSrc), "the chain layer routes through Pons");
ok(!/deployLauncher/.test(chainSrc), "and carries no launcher of its own to fall back to");
ok(/watchLaunches/.test(chainSrc), "launches are watched for rather than waited on");
