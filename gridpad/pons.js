 ...
 *
 * Gridpad does not need its own launcher on a chain Pons is on. Pons mints the
...
const PONS = {
   ...
   * equities; a coin paired to a power station trades in ETH. */
   ...
   * and no more.
   */
  OVERRIDE_KEY: id => `gridpad.pons.factory.${id}`,
   ...
   */
  TAG: /\[gridpad:1:([A-Z0-9]{1,12})\]/,

  /* Everything a person needs to go and check the pairing, then the tag an
   * indexer reads. The figures are the register's, which are the Global Power
   * Plant Database's: capacity as reported, coordinates as recorded, and the
   * body that published them named in the sentence. A claim somebody can
   * check is the only kind worth writing into a contract. */
  describe(source, place, note) {
    const head = place ? `Paired to ${place} (${source}).` : `Paired to ${source}.`;
    const body = note ? ` ${note}` : "";
    return `${head}${body} The pairing names that power station and nothing more:` +
      ` it conveys no ownership of the plant, no claim on its output or its revenue,` +
...
const PonsAdapter = {
  ...
  knownKey() { return `gridpad.pons.known.${this.chain.chainId}`; },
  known() {
