...
const SIZES = [
  ...
  ["header-x",      1500,  500, 54,   17,   36,  270,   88,  "40% 58%", "right"],
...
(async () => {
  ...
  for (const [name, w, h, title, body, mark, padL, padR, pos, side] of SIZES) {
    const p = await b.newPage({ viewport: { width: w, height: h }, deviceScaleFactor: 1 });
    await p.goto("file://" + __dirname + "/header.html");
    await p.evaluate(([fonts, w, h, title, body, mark, padL, padR, pos, side]) => {
      ...
      // The scrim leans to whichever side the type sits on; leaving it on the
      // left with the text on the right puts the words over the bright half.
      document.getElementById("scrim").style.background = side === "right"
        ? `linear-gradient(260deg, rgba(48,18,30,.90) 0%, rgba(48,18,30,.72) 30%,
      ...
      if (side === "right") {
        // The avatar sits bottom-left, so this side needs no inset for it
        inn.style.padding = `0 ${padL}px 0 ${padR}px`;
        inn.style.alignItems = "flex-end";
        inn.style.textAlign = "right";
        document.querySelector(".mark").style.justifyContent = "flex-end";
        document.getElementById("rule").style.marginLeft = "auto";
        document.getElementById("p").style.marginLeft = "auto";
      } else {
    ...
    }, [fonts, w, h, title, body, mark, padL, padR, pos, side]);
