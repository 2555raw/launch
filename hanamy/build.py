...
mw compare <span class="k">-a</span> claude-opus-5 <span class="k">-b</span> gemini-3-pro <span class="s">"explain a bonding curve"</span></pre>
    ...
    <h3>The hosted endpoint</h3>
    <p>A deployment can carry one key of its own, set as <code>HANAMY_KEY</code> (and
      <code>HANAMY_UPSTREAM</code> if the provider is not OpenAI). The browser never sees it:
      requests go to this site's <code>/v1</code>, which adds the key and forwards them. Nothing
      ...
      still works and simply asks each visitor for an endpoint of their own.</p>
...
<div class="talk">
  ...
  <div class="wire" id="t-wire" hidden>
    <div class="wire-in">
      <div class="wire-why" id="t-why">
        <p><b>Hanamy is the interface, not the model.</b> The answers come from an AI provider,
          and every provider charges for them, so a key has to come from somewhere. Two ways:</p>
        <ol>
          <li><b>Your own key, here.</b> Get one from
            ...
            or anything else that speaks the OpenAI format, paste it below and press Save. It stays
            in this browser and is sent nowhere but the endpoint you name.</li>
          <li><b>One key for everybody.</b> Whoever runs this deployment sets
            <code>HANAMY_KEY</code> on the server. Then nobody is asked for anything, and the key
            never reaches any browser. This copy does not have one.</li>
        </ol>
      </div>
      <p class="sub" id="t-why-hosted" hidden>This site carries its own key, so you need nothing.
        You can still point it somewhere else below; that setting stays in this browser.</p>
      <div class="wire-row">
