...
`contracts/Hydropad.sol` holds two contracts:

- **`HydropadToken`**: a plain ERC-20 that also stores its water source ticker.
  The whole supply is minted to the launcher at birth; there is no mint function.
- **`Hydropad`**: the launcher. It creates pairings, runs each curve, and keeps
...
Hydropad is not a service. If the network your wallet is on has no launcher, the
...
| File | What it is |
| --- | --- |
| `contracts/Hydropad.sol` | The launcher and the token |
