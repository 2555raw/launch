...
contract Hydropad {
    ...
    function launch(
        ...
        token = address(new HydropadToken(name, symbol, source, supply));
    ...
    function sell(address token, uint256 tokenAmount, uint256 minEthOut) external nonReentrant {
        ...
        require(HydropadToken(token).transferFrom(msg.sender, address(this), tokenAmount), "transferFrom");
    ...
    function _buy(address token, uint256 ethIn, uint256 minTokensOut) internal {
        ...
        require(HydropadToken(token).transfer(msg.sender, tokensOut), "transfer");
