...
const SIZES = [
  // name            w     h    title  body  mark  gutter  where the photo sits
  ["header-x",      1500,  500, 62,   19,   40,   88,  "52% 58%"],
  ["header-og",     1200,  630, 66,   20,   42,   80,  "52% 60%"],
  ["header-wide",   2400,  800, 98,   30,   64,  140,  "52% 58%"],
  ["header-square", 1200, 1200, 82,   26,   56,   96,  "50% 55%"],
];
...
(async () => {
  ...
  for (const [name, w, h, title, body, mark, pad, pos] of SIZES) {
    const p = await b.newPage({ viewport: { width: w, height: h }, deviceScaleFactor: 1 });
    await p.goto("file://" + __dirname + "/header.html");
    await p.evaluate(([fonts, w, h, title, body, mark, pad, pos]) => {
    ...
    }, [fonts, w, h, title, body, mark, pad, pos]);
