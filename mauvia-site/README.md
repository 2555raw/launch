...
Marketing site and interface prototype for **Mauvia**, a Uniswap v4 hook that reprices a
...
under `mauvia-legal-v1`. Clear site data to see it again.
...
```bash
docker build -t mauvia .
docker run --rm -p 8080:8080 mauvia
```
...
Root Directory to `/mauvia-site`, push, and Railway redeploys on its own.
