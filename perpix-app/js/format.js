...
/* Perpix keeps one clock, and it is Paris's.
   A market has a single wall clock, not one per viewer: two people looking at
   the same funding window have to be looking at the same hour, so the venue
...
export const MARKET_TZ = 'Europe/Paris';
export const MARKET_TZ_LABEL = 'Paris';

/* 24-hour, because a trading clock is read in a column next to other figures
   and AM/PM both changes width and leaves noon ambiguous. Dates read the way
   they are written where the clock is. */
...
/** The offset Paris is on right now, so the clock says which one it is rather
