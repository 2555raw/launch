/* ===========================================================================
   Propello — page behaviour. No dependencies.
   =========================================================================== */
(function () {
  'use strict';
  ...
  var STRINGS = {
    en: {
      ...
      chartAria: function (a, b, p) { return 'Line chart of the vPROP share price rising from 1.0000 euro on ' + a + ' to ' + p + ' at the close of ' + b + '. The figures behind it are listed in The Roll table above.'; },
      note: function (d, sh, n, p4, w, r, c) { return 'Deposited on ' + d + ' at €1.0000, that is <b>' + sh + ' vPROP</b>. After ' + n + ' closes vPROP is ' + p4 + ', so it is worth <b>' + w + '</b> — ' + r + ' of rent the vault kept, and ' + c + ' from the curve tax. Nothing was paid out along the way.'; },
      cities: {}, countries: {},
    ...
    zh: {
      ...
      chartAria: function (a, b, p) { return 'vPROP 份额价格折线图：从 ' + a + ' 的 €1.0000 升至 ' + b + ' 结算时的 ' + p + '。背后的数字列在上方的月报表中。'; },
      note: function (d, sh, n, p4, w, r, c) { return '于 ' + d + ' 以 €1.0000 存入，即 <b>' + sh + ' vPROP</b>。' + n + '次结算后 vPROP 为 ' + p4 + '，因此价值 <b>' + w + '</b>——其中 ' + r + ' 是金库留存的租金，' + c + ' 来自曲线税。过程中没有任何派发。'; },
  ...
  var CITIES = window.PROPELLO_CITIES || {};
  var FACADES = window.PROPELLO_FACADES || {};
  ...
  function render(amount) {
    ...
    oShares.textContent = shares.toLocaleString(LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' vPROP';
