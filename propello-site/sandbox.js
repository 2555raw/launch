/* ===========================================================================
   Propello — the sandbox. The vault's rules (contracts/Vault.sol) run here in
...
(function () {
  ...
  if (!D) { console.error('Propello: data/vault.js did not load'); return; }
  ...
  var STR = {
    en: {
      ...
      depOk: function (a, s, p) { return 'Deposited ' + a + ' at ' + p + ': ' + s + ' vPROP.'; },
      redNone: 'Nothing to redeem.',
      redMore: 'You do not hold that many vPROP.',
      redOk: function (n, t) { return 'Redeemed: ' + n + ' paid out' + (t ? ', ' + t + ' curve tax stayed in the vault' : '') + '.'; },
      redQueued: function (s) { return 'The reserve cannot cover it right now: ' + s + ' vPROP queued for the next close.'; },
      ...
      depPreview: function (s) { return '→ ' + s + ' vPROP'; },
      ...
      queueNote: function (s) { return s + ' vPROP of yours are queued. They settle at the next close, at that close’s price.'; },
      ...
      ledger: {
        ...
        deposit: function (w, a, s, p) { return w + ' deposited ' + a + ' for ' + s + ' vPROP at ' + p + '.'; },
        redeem: function (w, s, n, t) { return w + ' redeemed ' + s + ' vPROP for ' + n + (t ? ' (' + t + ' curve)' : '') + '.'; },
        queue: function (w, s) { return w + ' queued ' + s + ' vPROP for the close.'; },
        settle: function (w, s, n) { return 'The close paid ' + w + ' ' + n + ' for ' + s + ' queued vPROP.'; },
    ...
    zh: {
      ...
      cancelled: '已在钱包中取消。',
      couldNot: '无法连接：',
      signMsg: function (a) { return 'Propello 沙盒\n\n签名以在沙盒中使用 ' + a + '。\n没有交易，没有 gas，不会上链。'; },
      ...
      depOk: function (a, s, p) { return '以 ' + p + ' 存入 ' + a + '：' + s + ' vPROP。'; },
      redNone: '没有可赎回的份额。',
      redMore: '你没有那么多 vPROP。',
      redOk: function (n, t) { return '已赎回：支付 ' + n + (t ? '，' + t + ' 曲线税留在金库' : '') + '。'; },
      redQueued: function (s) { return '储备目前不足以支付：' + s + ' vPROP 已排队等待下次结算。'; },
      ...
      queueNote: function (s) { return '你有 ' + s + ' vPROP 在排队，将在下次结算时按当时价格结清。'; },
      ...
      ledger: {
        ...
        deposit: function (w, a, s, p) { return w + ' 以 ' + p + ' 存入 ' + a + '，得到 ' + s + ' vPROP。'; },
        redeem: function (w, s, n, t) { return w + ' 赎回 ' + s + ' vPROP，得到 ' + n + (t ? '（曲线税 ' + t + '）' : '') + '。'; },
        queue: function (w, s) { return w + ' 将 ' + s + ' vPROP 排队等待结算。'; },
        settle: function (w, s, n) { return '结算向 ' + w + ' 支付 ' + n + '，对应 ' + s + ' 排队的 vPROP。'; },
  ...
  var LS_KEY = 'propello.sandbox.v1';
  ...
  store.subscribe = function (cb) {
    listeners.push(cb);
    if (db) {
      db.doc('sandbox/vault').onSnapshot(function (snap) { cb(snap.exists ? snap.data() : seed()); }, function (e) { console.warn('Propello sandbox: store lost', e); });
    } else cb(localRead());
  };
  ...
  }).catch(function (e) { console.warn('Propello sandbox: capabilities unavailable', e); }).then(start);
})();
