...
/* ===========================================================================
   ...
   Writes data/cities.js (window.PROPELLO_CITIES) and data/cities.json.
   =========================================================================== */
'use strict';
const fs = require('fs');
...
for (const [city, c] of Object.entries(SCHEMATIC)) {
  ...
  let districts, limitPts, outline = null;
  ...
  if (real) {
    ...
    /* a real municipal outline, when one is on disk, becomes the city limit:
       the schematic cells are drawn inside the true shape of the city */
    const of = path.join(root, 'data', 'geo', city.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '') + '-outline.geojson');
    outline = fs.existsSync(of) ? JSON.parse(fs.readFileSync(of, 'utf8')).features[0].geometry : null;
    ...
    let limit = hull(ring);
    let outlineRings = null;
    if (outline) {
      outlineRings = (outline.type === 'Polygon' ? [outline.coordinates] : outline.coordinates).map(poly => poly[0].map(([lon, lat]) => proj(lon, lat)));
      const pts = outlineRings.flat();
      limit = hull(pts.map(p => [p[0] * 1.02, p[1] * 1.02]));     /* cells reach a hair past the outline; the SVG clips them to it */
    }
    const cells = voronoi(sites, limit);
    districts = c.districts.map(([name], i) => ({ name, rings: [cells[i]], biggest: cells[i] }));
    limitPts = outlineRings ? outlineRings.flat() : limit;
    if (outlineRings) outline = outlineRings;
  }
  ...
  const outlinePath = outline ? pathOf(outline.map(r => simplify(r.map(T), 0.9))) : null;
  out[city] = {
    country: c.country, region: c.region, real: !!real, outline: !!outline, viewBox: `0 0 ${W} ${H}`,
    limit: real ? '' : (outlinePath || pathOf([hull(limitPts).map(T)])),
    scaleKm: km, scalePx: Math.round(km / kmPerPx),
    districts: drawn.map(({ _big, ...d }) => d),
    buildings
  };
  console.error((real ? '  ✓ real  ' : outline ? '  ◐ outline' : '  ~ schema') + ' ' + city + ': ' + drawn.length + ' districts, ' + buildings.map(b => b.district).join(' / '));
}
fs.writeFileSync(path.join(root, 'data', 'cities.js'), '/* generated by scripts/cities.js — do not edit */\nwindow.PROPELLO_CITIES = ' + JSON.stringify(out) + ';\n');
