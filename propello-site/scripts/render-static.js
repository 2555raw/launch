...
const LANGS = {
  en: {
    ...
    chartSub: (a, b) => `${a} → ${b}`,
    figs: ['Share price', 'Since 1 ', 'vPROP in issue', 'What the vault holds', 'Deposit fee', 'Exit fee'],
    calc: ['vPROP you would hold', 'Worth today', 'Of which rent', 'Of which the curve tax'],
    ledger: ['Rent scheduled by the leases', 'Rent collected', (city, d) => `(one lease in ${city} ended on the ${d})`, 'Property costs and fees', 'Kept, and added to the reserve', 'Curve tax received from early redemptions', 'Shares in issue at the close', 'Change in the share price', 'Closing price of one vPROP'],
  ...
  zh: {
    ...
    chartSub: (a, b) => `${a} → ${b}`,
    figs: ['份额价格', '自 3 月 1 日起 ', 'vPROP 流通量', '金库持有', '存入费', '赎回费'],
    calc: ['你将持有的 vPROP', '今日价值', '其中租金', '其中曲线税'],
    ledger: ['租约应收租金', '实收租金', (city, d) => `（${city}的一份租约于 ${d}到期）`, '物业成本与费用', '留存并计入储备', '提前赎回缴纳的曲线税', '结算时流通份额', '份额价格变动', '一个 vPROP 的结算价格'],
...
function render(lang) {
  ...
  const D = 10000;
  R.calc = `<div class="rt-row"><span>${L.calc[0]}</span><b class="rt-num" id="o-shares">${num(D, 2)} vPROP</b></div>
