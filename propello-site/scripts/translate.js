...
function repath(html) {
  ...
  html = html.replace(/<link rel="canonical" href="https:\/\/propello\.example\/([^"]*)">/, (m, p) => '<link rel="canonical" href="https://propello.example/' + LANG + '/' + p + '">');
  html = html.replace(/<meta property="og:url" content="https:\/\/propello\.example\/([^"]*)">/, (m, p) => '<meta property="og:url" content="https://propello.example/' + LANG + '/' + p + '">');
...
function leftovers(html, skipData) {
  ...
  const ok = /^(Propello|vPROP|EURG|Docs|X|EN|ES|N|km|USDG|ERC-4626|SHA-256|CSV|B\.V\.|KvK|CET)$/;
