/* ===========================================================================
   Propello — wallet connection. No library: EIP-6963 discovery, EIP-1193
   ...
   What it does:
     ...
     · with a vault address configured, reads your vPROP balance, the share
...
(function () {
  ...
  var T = ZH ? { connect: '连接钱包', wallet: '钱包', network: '网络', switchTo: '切换到 ', noVault: function (ch) { return ch + ' 上尚未部署金库。合约源码在 <code>contracts/</code>；把地址填进 <code>config.js</code>，此面板就会从合约读取你的余额。'; }, explorer: '在区块浏览器中查看', disconnect: '断开', yours: '你的 vPROP', worth: '价值', price: '份额价格', holds: '金库持有', readFail: '无法读取金库：', noWallet: '未找到钱包。请安装 MetaMask、Rabby 或 Coinbase Wallet 后重试。', connected: '已连接 ', via: '，通过 ', cancelled: '已取消连接。', couldNot: '无法连接：', disconnected: '已断开。钱包自身的授权仍然保留。' }
        : { connect: 'Connect wallet', wallet: 'Wallet', network: 'Network', switchTo: 'switch to ', noVault: function (ch) { return 'No vault is deployed on ' + ch + ' yet. The contract source is in <code>contracts/</code>; set its address in <code>config.js</code> and this panel reads your balance from it.'; }, explorer: 'View on explorer', disconnect: 'Disconnect', yours: 'Your vPROP', worth: 'Worth', price: 'Share price', holds: 'Vault holds', readFail: 'Could not read the vault: ', noWallet: 'No wallet found. Install MetaMask, Rabby or Coinbase Wallet and try again.', connected: 'Connected ', via: ' via ', cancelled: 'Connection cancelled.', couldNot: 'Could not connect: ', disconnected: 'Disconnected. The wallet itself keeps its own permissions.' };
  ...
  function readVault() {
    ...
    }).catch(function (err) { console.warn('Propello: vault read failed', err); return ['<p class="rt-wp-note">' + T.readFail + (err.message || err) + '</p>']; });
  }
  ...
  window.PROPELLO_WALLET = { connect: connect, disconnect: disconnect, state: state };
  $$('[data-connect]').forEach(function (b) {
    if (b.tagName !== 'BUTTON') return;
