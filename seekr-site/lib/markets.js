...
async function refreshSeekr() {
  ...
  try {
    ...
    if (p) seekr = { at: Date.now(), stale: false, live: true, price: Number(p.priceUsd), change: Number(p.priceChange?.h24 || 0), mcap: Number(p.marketCap || p.fdv || 0) || null, liquidity: Number(p.liquidity?.usd || 0) || null, volume: Number(p.volume?.h24 || 0) || null, symbol: 'SEEKR', url: p.url, chainId: p.chainId, dexId: p.dexId };
