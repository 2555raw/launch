/* Vesica — the vault list, the deposit drawer and the menu.
   The table itself and the formatting live in vaults-data.js, which this page
   and the single-vault page share. */
  ...
  rows.innerHTML = list.map(v => {
    ...
    <details class="cs-row" data-t="${v.t}"${open.has(v.t) ? ' open' : ''}>
      ...
      <a class="cs-open" href="vault.html?v=${v.t}">Open the full page for this vault
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 12h13M13 7l5 5-5 5"/></svg></a>
    </details>`;
  }).join('');
